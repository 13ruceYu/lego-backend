...
export default class UserService extends Service {
  ...
  async loginByCellphone(cellphone: string) {
    ...
    const token = sign({ username: newUser.username, _id: newUser._id }, app.config.jwt.secret, { expiresIn: app.config.jwtExpires });
    return token;
  }
  ...
  async loginByGithub(code: string) {
    ...
    // 如果用户存在
    if (existUser) {
      const token = sign({ username, _id: existUser._id }, app.config.jwt.secret, { expiresIn: app.config.jwtExpires });
      return token;
    }
    // 如果用户不存在
    const userCreateData: Partial<IUserProps> = {
